...
public class PassiveIncomeStrategy extends BaseHandler implements PassiveIncomeOptionProcessor {
	...
	class OptionPositions {
		...
		public Map<String, Integer> optionsWithNetShortAndLong = null;
		...
		public Map<String, Integer> getOptionsWithNetShortAndLong() {
	...
	public void closeOptionsThatAreInProfitAndPotentiallyOpenNewOnes(String accountId) {
		...
		try {
			...
			purchaseLongOptionsIfNeeded(optionsWithoutProtection, prices, accountId);
	...
	protected void purchaseSellCallOrPutIfLongOptionsExist(OptionPositions op, Map<String, Double> prices,
			String accountId) {
		for (Map.Entry<String, Integer> entry : op.getOptionsWithNetShortAndLong().entrySet()) {
			...
			purcahseSellCallOrPutIfLongOptionExists(od, prices.get(od.getStockTicker()), accountId, value, 1);
		}

	}

	private void purcahseSellCallOrPutIfLongOptionExists(OptionData od, Double stockPrice, String accountId,
		...
		if( Util.isLastFewHoursOfTrading(2)) {
			System.out.println("Not Executing purchaseSellCallOrPutIfLongOptionExists -> "+od.getKeyWithoutStrikePrice()+" -> "+accountId+" -> "+value);
			return;
		}
		if (value > 0) {
			System.out.println("purchaseSellCallOrPutIfLongOptionExists -> "+od.getKeyWithoutStrikePrice()+" -> "+accountId);
			placeNextSmartLongOptionOrder(accountId, od.createOptionSymbolForPrice(stockPrice.doubleValue()), quantity, od.getPutCall());
	...
	protected double potentialProfitForOptionPosition(Position p) {
		...
		if (shortQuantity > 0 /* && oi.getPutCall() == OptionInstrument.PutCall.PUT */) {
			potentialProfitMarketValue = getPotentialProfitValueForShortPosition(shortQuantity, p);
		...
		if (longQuantity > 0) {
			potentialProfitMarketValue = getPotentialProfitMarketValueForLongPositions(longQuantity, p);
			// pop = isOptionInProfit(marketValue, potentialProfitMarketValue, oi,
			// currentStockPrice);
		}
	...
	public void closeOptionIfInProfitAndPotentiallyOpenNewOne(PassiveIncomeOptionProcessorInput input) {
		...
		OptionInstrument oi = (OptionInstrument) p.getInstrument();
		String symbol = oi.getSymbol();
		String high = op.getAdjacentHigherSymbol(symbol);
		String low = op.getAdjacentLowerSymbol(symbol);
		if (high != null || low != null) {
			// System.out.println("Symbol with protection -> [low,symbol,high][" + low + ","
			// + symbol + "," + high + "]");
		} else {
			// System.out.println("Symbol with no protection -> " + symbol);
		}
		...
		int longQuantity = p.getLongQuantity().intValue();
		double marketValue = p.getMarketValue().doubleValue();
		// System.out.println("Current value -> "+oi.getSymbol()+" -> "+marketValue+" ->
		...
		if (shortQuantity > 0) {
			...
			// System.out.println("Symbol's percentage return[" + percentage + "] of
			// purchase price -> " + symbol + " -> "
			// + purchasePrice + " -> " + market);
			if (market < 0.05 * purchasePrice) {
				// System.out.println("Symbol has reached 5% of purchase price -> "+symbol+" ->
				// "+purchasePrice +" -> "+market);
				if (low == null) {
					...
					System.out.println("New Low < 0.05 -> " + newLow + " -> for -> " + symbol + " -> " + purchasePrice
							+ " -> " + market + " -> " + percentage);
					placeProtectionLongOrder(accountId, newLow, OptionSymbolParser.parse(newLow).getPutCall(),
							shortQuantity);
				}
				...
				if (low == null) {
					// String newLow =
					// optionSymbolManager.getAdjacentOptionSymbols(symbol).adjacentLowerSymbol;
					// System.out.println("New Low > 0.05 -> " + newLow + " -> for -> " + symbol);
					// placeProtectionLongOrder(accountId, newLow,
					// OptionSymbolParser.parse(newLow).getPutCall(),
					// shortQuantity);
				}
				// System.out.println("Symbol didn't reach 5% of purchase price -> "+symbol+" ->
				// "+purchasePrice +" -> "+market);
			}
		}
	...
	protected void placeNextSmartLongOptionOrderOld(String accountId, String symbol, int quantity,
		...
		try {
			...
			System.out.println("placeNextSmartLongOptionOrder -> Happy Path - > " + Util.toJSON(order));
			getClient().placeOrder(accountId, order);
		} catch (RuntimeException e) {
			order = Util.makeSmartOptionWhenTheSymbolDoesntExist(symbol, quantity, Duration.DAY, null, pc,
					OrderType.MARKET, Instruction.BUY_TO_OPEN);
			System.out.println("placeNextSmartLongOptionOrder -> No Option exists path - > " + Util.toJSON(order));
			getClient().placeOrder(accountId, order);
		}
	}
	...
	protected void placeNextSmartMarketOptionOrder(String accountId, String symbol, int quantity,
			OptionInstrument.PutCall pc, Instruction i) {
		if (!Util.canOptionsBeTradedNow()) {
			System.out.println("This is not market hours for trading options and I can't place market orders -> "
					+ accountId + " -> " + symbol + " -> " + quantity);
			return;
	...
	protected void openNewOptionPositionForSellCallOrPut(String accountId, Ticker ticker, int shortQuantity,
		...
		if (opc == OptionInstrument.PutCall.CALL) {
			if (shouldPlaceTradeForNextDayOrWeekOptions(ticker)) {
				...
				// Util.toJSON(p));
				placeNextDayTradeForPassiveIncome(accountId, ticker, 3f, null, shortQuantity);

			} else {
				// System.out.println("Opening a new sell call for Current Day -> " +
				// Util.toJSON(p));
				placeDailyTradeForPassiveIncome(accountId, ticker, 3f, null, shortQuantity);
			}
		}
		if (opc == OptionInstrument.PutCall.PUT) {
			if (shouldPlaceTradeForNextDayOrWeekOptions(ticker)) {
				...
				// Util.toJSON(p));
				placeNextDayTradeForPassiveIncome(accountId, ticker, null, 3f, shortQuantity);
			} else {
				// System.out.println("Opening a new sell put for Current Day -> " +
				// Util.toJSON(p));
				placeDailyTradeForPassiveIncome(accountId, ticker, null, 3f, shortQuantity);
			}
		}
	...
	public void placeNextLongOptionOrderXXX(String accountId, OptionInstrument oi, int shortQuantity, int longQuantity,
			PossibleOptionAndProfit pop, PassiveIncomeOptionProcessorInput input) {
		Order order = null;
		String o = null;
		...
		try {
			...
			e.printStackTrace();
			order = Util.makeSmartOptionWhenTheSymbolDoesntExist(o, getQuantity(shortQuantity, longQuantity),
					Duration.DAY, null, oi.getPutCall(), OrderType.MARKET, pop.instruction);
			System.out.println(Util.toJSON(order));
			getClient().placeOrder(accountId, order);
		}
	}
