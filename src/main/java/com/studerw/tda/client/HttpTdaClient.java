...
/**
 * HTTP implementation of {@link TdaClient} which uses OKHttp3 under the hood
 * and uses the new OAuth based security. <strong>This is a thread safe
 * class.</strong>
 *
 * @see <a href="https://square.github.io/okhttp/3.x/okhttp/">OKHttp3 from
 *      Square</a>
 */
public class HttpTdaClient implements TdaClient {
	 ...
	 * <li>tda.client_id (or sometimes referenced as <em>Consumer Key</em> and it
	 * should not have <em>@AMER.OAUTHAP</em> appended</li>
	 * <li>tda.url=<em>https://api.tdameritrade.com/v1</em></li>
	 * <li>tda.debug.bytes.length=<em>-1</em> (How many bytes of logging interceptor
	 * debug to print, -1 is unlimited)</li>
	 * </ul>
	 *
	 * <p>
	 * There are no defaults for the <em>tda.token.refresh</em> and
	 * <em>tda.client_id</em> (your consumer key). If they are not set, an exception
	 * will be thrown Note that the client id should not have appended the
	 * <em>@AMER.OAUTHAP</em> part that is used when refreshing your OAuth token.
	 * </p>
	 */
	 ...
	 *
	 * <p>
	 * There are no defaults for <em>tda.token.refresh</em> and
	 * <em>tda.client_id</em> (<em>consumer key)</em>. If they are not set, an
	 * exception will be thrown. Note that sometimes TDA uses <em>Consumer Key</em>
	 * instead of the term <em>client id</em>. They are the same. The client id
	 * should not have appended the <em>@AMER.OAUTHAP</em> part that is used when
	...
	public HttpTdaClient(Properties props) {
		...
		this.httpClient = new OkHttpClient.Builder().cookieJar(new CookieJarImpl(new MemoryCookieStore()))
				.addInterceptor(new OauthInterceptor(this, tdaProps)).addInterceptor(new LoggingInterceptor("TDA_HTTP",
						Integer.parseInt(tdaProps.getProperty("tda.debug.bytes.length"))))
				.build();
	}

	protected static Properties initTdaProps() {
		try (InputStream in = HttpTdaClient.class.getClassLoader().getResourceAsStream("tda-api.properties")) {
	...
	/**
	 * validates the necessary props like refresh token and client id (consumer
	 * key). If others are missing, just use friendly defaults.
	 *
	...
	public PriceHistory priceHistory(PriceHistReq priceHistReq) {
		...
		if (priceHistReq.getExtendedHours() != null) {
			urlBuilder.addQueryParameter("needExtendedHoursData", String.valueOf(priceHistReq.getExtendedHours()));
		}

		Request request = new Request.Builder().url(urlBuilder.build()).headers(defaultHeaders()).build();
	...
	public List<Quote> fetchQuotes(List<String> symbols) {
		...
		HttpUrl url = baseUrl("marketdata", "quotes").addQueryParameter("symbol", String.join(",", symbols)).build();

		Request request = new Request.Builder().url(url).headers(defaultHeaders()).build();
		String requestJSON = Util.toJSON(request);
		//System.out.println("Request -> "+requestJSON);
		//System.out.println("HttpUrl -> "+Util.toJSON(url));
		try (Response response = this.httpClient.newCall(request).execute()) {
			checkResponse(response, false);
	...
	public void placeOrder(String accountId, Order order) {
		...
		//System.out.println("Placing Order -> "+json);
		RequestBody body = RequestBody.create(MediaType.parse("application/json"), json);
		Request request = new Request.Builder().url(url).headers(defaultHeaders()).post(body).build();
	...
	public void replaceOrder(String accountId, Order order, String orderId) {
		...
		RequestBody body = RequestBody.create(MediaType.parse("application/json"), json);
		Request request = new Request.Builder().url(url).headers(defaultHeaders()).put(body).build();
	...
	public List<Order> fetchOrders(String accountId, OrderRequest orderRequest) {
		...
		if (orderRequest.getToEnteredTime() != null) {
			urlBuilder.addQueryParameter("toEnteredTime", Utils.toTdaISO8601(orderRequest.getToEnteredTime()));
		}
		if (orderRequest.getFromEnteredTime() != null) {
			urlBuilder.addQueryParameter("fromEnteredTime", Utils.toTdaISO8601(orderRequest.getFromEnteredTime()));
		}
	...
	public void cancelOrder(String accountId, String orderId) {
		...
		Request request = new Request.Builder().url(url).headers(defaultHeaders()).delete().build();
	...
	public List<Instrument> queryInstruments(Query query) {
		...
		if (query == null || StringUtils.isEmpty(query.getSearchStr()) || query.getQueryType() == null) {
			throw new IllegalArgumentException("The instrument query must have both a searchStr and QueryType set.");
		}
		HttpUrl url = baseUrl("instruments").addQueryParameter("symbol", query.getSearchStr())
				.addQueryParameter("projection", query.getQueryType().getQueryType()).build();
	...
	public FullInstrument getFundamentalData(String id) {
		...
		HttpUrl url = baseUrl("instruments").addQueryParameter("symbol", id)
				.addQueryParameter("projection", "fundamental").build();
		...
		try (Response response = this.httpClient.newCall(request).execute()) {
			...
			if (fullInstruments.size() != 1) {
				throw new RuntimeException("Expecting a single instrument but received: " + fullInstruments.size());
			}
	...
	public OptionChain getOptionChain(OptionChainReq chainRequest) {
		...
		if (chainRequest.getFromDate() != null) {
			urlBuilder.addQueryParameter("fromDate",
					chainRequest.getFromDate().format(DateTimeFormatter.ISO_DATE_TIME));
		}
	...
	public OptionChain getOptionChain(String symbol) {
		...
		OptionChainReq request = OptionChainReq.Builder.optionChainReq().withSymbol(symbol).build();
	...
	public List<Transaction> fetchTransactions(String accountId, TransactionRequest request) {
		...
		Request httpReq = new Request.Builder().url(urlBuilder.build()).headers(defaultHeaders()).build();
	...
	public Transaction getTransaction(String accountId, Long transactionId) {
		...
		Builder urlBuilder = baseUrl("accounts", accountId, "transactions", String.valueOf(transactionId));
	...
	public Instrument getInstrumentByCUSIP(String id) {
		...
		HttpUrl url = baseUrl("instruments", id).addQueryParameter("fundamental", "true").build();

		Request request = new Request.Builder().url(url).

				headers(defaultHeaders()).build();
	...
	protected StreamerSubscriptionKeys getSubscriptionKeys(List<String> accountsIds) {
		...
		Builder urlBuilder = baseUrl("userprincipals", "streamersubscriptionkeys").addQueryParameter("accountIds",
				String.join(",", accountsIds));
	 ...
	 * @param emptyJsonOk is an empty JSON object or array actually OK (e.g.
	 *                    fetchMovers)?
	 */
	private void checkResponse(Response response, boolean emptyJsonOk) {
		if (!response.isSuccessful()) {
			...
			String msg = String.format("Non 200 response:  [%d - %s] - %s", response.code(), errorMsg,
		...
		if (!emptyJsonOk) {
			try {
				...
				if ("{}".equals(json) || "[]".equals(json)) {
					String msg = String.format("Empty json body:  [%d - %s] - %s", response.code(), response.message(),
							response.request().url());
					throw new RuntimeException(msg);
				}
			} catch (IOException e) {
